...
def display_performance_metrics(trues, predicted, probs, class_list, positive_label=None):
    class_metrics, general_metrics, roc, conf_matrix = calculate_performance_metrics(
        trues, predicted, probs, class_list, positive_label=positive_label)
...
def save_performance_metrics(trues, predicted, probs, class_list, folder: Path, positive_label=None):
    class_metrics, general_metrics, roc, conf_matrix = calculate_performance_metrics(
        trues, predicted, probs, class_list, positive_label=positive_label)
    ...
    doc = f'<div>{general_metrics_f.to_html()}</div><div>{conf_matrix_f.to_html()}</div>' \
          f'<div>{class_metrics_f.to_html()}</div><div><img src = "roc.svg" /></div><div>{roc.to_html()}</div>' \
          f'<div>{DataFrame(probs).to_html()}</div>'
...
def print_performance_metrics(trues, predicted, probs, class_list, positive_label=None):
    class_metrics, general_metrics, roc, conf_matrix = calculate_performance_metrics(
        trues, predicted, probs, class_list, positive_label=positive_label)
...
def calculate_performance_metrics(trues, predicted, probs, class_list, positive_label=None):
    """
    ...
    :param positive_label: name of the class considered the positive class (for ROC)
    ...
    if probs is not None:
        if positive_label is None:
            positive_label = trues[0]
        from sklearn.metrics import roc_auc_score, roc_curve
        fpr, tpr, thresholds = roc_curve(y_true=trues, y_score=probs, pos_label=positive_label)
        roc = df({'fpr': fpr, 'tpr': tpr})
        roc_auc = (roc_auc_score([l == positive_label for l in trues], probs))
    else:
        roc = None
        roc_auc = None
